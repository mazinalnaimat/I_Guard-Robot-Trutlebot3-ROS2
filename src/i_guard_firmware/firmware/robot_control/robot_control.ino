...
#define pinGasSensor A6
#define Buzzer  A5
#define pinIsThereGasTo_rpi 6

double GasSensorValue = 0;
...
void setup() {
  ...
  digitalWrite(L298N_in1, LOW);//exchange with under (mazin)
  digitalWrite(L298N_in2, HIGH);
  digitalWrite(L298N_in3, LOW);//exchange with under (mazin)
...
void loop() {
  ...
  {
    right_wheel_meas_vel = (10 * right_encoder_counter * (60.0/(990))) * 0.10472;
    left_wheel_meas_vel = (10 * left_encoder_counter * (60.0/(990))) * 0.10472;
...
void chechGas()
{
 GasSensorValue = analogRead(pinGasSensor);

 if(GasSensorValue > 650)
 {
  tone(Buzzer, 2000);
  analogWrite(pinIsThereGasTo_rpi, 165);
 }
 else
 {
  noTone(Buzzer);
  analogWrite(pinIsThereGasTo_rpi, 0);
...
void rightEncoderCallback()
{
  ...
  {
    right_wheel_sign = "n";//exchange with else (mazin)
  }
  else
  {
    right_wheel_sign = "p";
...
void leftEncoderCallback()
{
  ...
  {
    left_wheel_sign = "p";//exchange with else (mazin)
  }
  else
  {
    left_wheel_sign = "n";
